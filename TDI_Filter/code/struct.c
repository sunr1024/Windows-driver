...
//@description  : Object hash table initialize.
NTSTATUS
ot_init(void)
{
    // g_addr_list
	...
	if (g_addr_list == NULL) {
		KdPrint(("~!![VMI_net] ot_init: malloc_np\n"));
...
//do cleannup for address & connection objects
void
ot_free(void)
{
    KIRQL irql;
               ...
               int ipproto)
{
	...
	for (ae = g_addr_list[hash]; ae != NULL; ae = ae->next)
        ...
        {
            KdPrint(("~!![VMI_net] add_addrobj: reuse addrobj 0x%x\n", addrobj));
            break;
        }
	...
	if (ae == NULL)
    {
		...
		if (ae == NULL) {
            KdPrint(("~!![VMI_net] ot_add_fileobj: malloc_np()!\n"));
...
find_addr_entry(PFILE_OBJECT addrobj, KIRQL *irql)
{
	...
	if (ae == NULL)
    {
		KdPrint(("~![VMI_net] find_addr_entry: fileobj 0x%x not found!\n", addrobj));
		KeReleaseSpinLock(&g_addr_list_lock, *irql);
	}
...
add_connobj(PFILE_OBJECT connobj, CONNECTION_CONTEXT conn_ctx)
{
    ...
    for (ce = g_conn_list[hash]; ce != NULL; ce = ce->next)
        ...
        {
            KdPrint(("~!![VMI_net] add_connobj: reuse connobj 0x%x\n", connobj));
            break;
        }
    ...
    if (ce == NULL)
    {
        ...
        if (ce == NULL) {
            KdPrint(("~!![VMI_net] add_connobj: malloc_np()!\n"));
            status = STATUS_INSUFFICIENT_RESOURCES;
            goto done;
        }
    ...
    find_conn_entry(PFILE_OBJECT connobj, KIRQL *irql)  // !if no delete -- g_conn_list_lock
{
    ...
    if (ce == NULL)
    {
        KdPrint(("~![VMI_net] find_conn_entry: connobj 0x%x not found!\n", connobj));
        KeReleaseSpinLock(&g_conn_list_lock, *irql);
    }
