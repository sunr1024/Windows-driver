...
tdi_send_datagram(PIRP irp, PIO_STACK_LOCATION irps, struct _completion *completion)
{
    ...
    if (get_original_devobj(irps->DeviceObject, &ipproto) == NULL ||
        (ipproto != IPPROTO_UDP && ipproto != IPPROTO_IP)) {
            ...
            KdPrint(("~![VMI_net] tdi_send_datagram: unknown DeviceObject 0x%x!\n",
                irps->DeviceObject));
            return result;
    }
    ...
    if (ae == NULL) {
        KdPrint(("~![VMI_net] tdi_send_datagram: ot_find_fileobj(0x%x)!\n", irps->FileObject));
#if DBG
    ...
    KdPrint(("[VMI_net] tdi_send_datagram(pid:%u/%u): addrobj 0x%x (size: %u) %x:%u -> %x:%u\n",
    ...
    if (r == NULL) {
        KdPrint(("~!![VMI_net] tdi_send_datagram: malloc_np\n"));
        goto done;
    }
...
{
	KdPrint(("[VMI_net] tdi_receive_datagram: addrobj 0x%x\n", irps->FileObject));
...
tdi_receive_datagram_complete(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context)
{
    ...
    {
		KdPrint(("~![VMI_net] tdi_receive_datagram_complete: unknown DeviceObject 0x%x!\n",
			DeviceObject));
		goto done;
	}

	KdPrint(("[VMI_net] tdi_receive_datagram_complete: addrobj 0x%x; status 0x%x; information %u\n",
		addrobj, Irp->IoStatus.Status, Irp->IoStatus.Information));

	if (Irp->IoStatus.Status != STATUS_SUCCESS) {
		KdPrint(("~![VMI_net] tdi_receive_datagram_complete: status 0x%x\n",
			Irp->IoStatus.Status));
		goto done;
	}
	...
	if (ae == NULL) {
		KdPrint(("~![VMI_net] tdi_receive_datagram_complete: ot_find_fileobj(0x%x)!\n",
			addrobj));
		goto done;
	}
    ...
    r = malloc_np(sizeof(flt_request));
    if (r == NULL) {
        KdPrint(("~!![VMI_net] tdi_receive_datagram_complete: malloc_np\n"));
        goto done;
    }
    memset(r, 0, sizeof(flt_request));
	...
	KdPrint(("[VMI_net] tdi_receive_datagram_complete(pid:%u): %x:%u -> %x:%u\n",
		ae->pid,
		ntohl(((TDI_ADDRESS_IP *)(remote_addr->Address))->in_addr),
		ntohs(((TDI_ADDRESS_IP *)(remote_addr->Address))->sin_port),
		ntohl(((TDI_ADDRESS_IP *)(local_addr->Address))->in_addr),
    ...
    OUT PIRP *IoRequestPacket)
{
    ...
    {
        // unknown device object!
        KdPrint(("[VMI_net] tdi_event_receive_datagram: unknown DeviceObject 0x%x!\n",
            ae));
	...
	KdPrint(("[VMI_net] tdi_event_receive_datagram(pid:%u) addrobj 0x%x: %x:%u -> %x:%u\n",
		ae->pid,
        ae->addrobj,
		ntohl(((TDI_ADDRESS_IP *)(remote_addr->Address))->in_addr),
		ntohs(((TDI_ADDRESS_IP *)(remote_addr->Address))->sin_port),
    ...
    r = malloc_np(sizeof(flt_request));
    if (r == NULL) {
        KdPrint(("~!![VMI_net] tdi_event_receive_datagram: malloc_np\n"));
        goto done;
    }
    memset(r, 0, sizeof(flt_request));
