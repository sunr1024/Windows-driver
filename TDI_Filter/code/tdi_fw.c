...
//     else
//     {
//         KdPrint(("~![VMI_net] get_original_devobj: Unknown DeviceObject 0x%x!\n", flt_devobj));
//     }
             ...
             IN WCHAR *pwch_devname)
{
    ...
    {
        KdPrint(("~![VMI_net] c_n_a_device fail: IoCreateDevice(%S): 0x%x\n", pwch_devname, status));
        return status;
    }
    ...
    if (status != STATUS_SUCCESS)
    {
        KdPrint(("~![VMI_net] c_n_a_device fail: IoAttachDevice(%S): 0x%x\n", pwch_devname, status));
        return status;
    }

    KdPrint(("[VMI_net] c_n_a_device: %-13S fltdevobj: 0x%x\n", pwch_devname, *ppFltDevObj));
            ...
            IN PUNICODE_STRING RegistryPath)
{
    ...
    KdPrint(("[VMI_net] &g_request_event: %x\n", &g_request_event));

	status = ot_init();
	if (status)
    {
		KdPrint(("~![VMI_net] DriverEntry: ot_init: 0x%x\n", status));
		goto done;
	}
	...
	if (status)
    {
		KdPrint(("~![VMI_net] DriverEntry: filter_init: 0x%x\n", status));
		goto done;
	}
	...
	if (status)
    {
		KdPrint(("~![VMI_net] DriverEntry: IoCreateDevice(control): 0x%x!\n", status));
		goto done;
	}
	...
	if (status)
    {
		KdPrint(("~![VMI_net] DriverEntry: IoCreateSymbolicLink: 0x%x!\n", status));
		goto done;
	}
	...
	if (status != STATUS_SUCCESS)
    {
		KdPrint(("~![VMI_net] DriverEntry: IoCreateDevice(nfo): 0x%x!\n", status));
		goto done;
	}
...
DeviceDispatch(IN PDEVICE_OBJECT DeviceObject, IN PIRP irp)
{
    ...
    {
		KdPrint(("~![VMI_net] DeviceDispatch: irp == null!\n"));
...
//             KdPrint(("[VMI_net] DeviceDispatch: IRP_MJ_DEVICE_CONTROL, CtlCode 0x%x, FileObject: 0x%08X\n",
...
//                 irpsp->FileObject));
			...
			if (status != STATUS_SUCCESS)
            {
				...
				if (buf != NULL && status == STATUS_SUCCESS)
                {
					...
					KdPrint(("[VMI_net] DeviceDispatch: IOCTL_TDI_QUERY_DIRECT_SEND_HANDLER: TCPSendData = 0x%x\n",
						g_old_TCPSendData));
        ...
        case IRP_MJ_INTERNAL_DEVICE_CONTROL:
            {
                ...
                {
//                     KdPrint(("[VMI_net] DeviceDispatch: MinorFunction: 0x%x, FileObject: 0x%x\n",
//                         irpsp->MinorFunction,
//                         irpsp->FileObject));
        ...
        if (irpsp->MajorFunction == IRP_MJ_DEVICE_CONTROL)
            {
                ...
                if (out_buf)
                {
                    ...
                    {
                        flt_request *request;
                        //KdPrint(("[VMI_net]  -- Flink:%x, Blink:%x\n", request_list_head.Flink, request_list_head.Blink));

					next:
						request = (flt_request *)ExInterlockedRemoveHeadList(&request_list_head, &request_list_lock);
    ...
    {
		KdPrint(("~![VMI_net] DeviceDispatch: ioctl for unknown DeviceObject 0x%x\n", DeviceObject));
                      ...
                      PVOID context)
{
    ...
    {
		KdPrint(("[VMI_net] tdi_dispatch_complete: [DROP!]"
    ...
    else if (filter == FILTER_ALLOW)  // ALLOW: pass IRP to the next driver
    {
        if (CompletionRoutine)
        {
            ...
            KdPrint(("[VMI_net] tdi_dispatch_complete[ALLOW]"
                "fltdevobj: 0x%x; FileObj 0x%x : major 0x%x, minor 0x%x.\n",
                fltdevobj,
                irps->FileObject,
                irps->MajorFunction,
...
g_new_TCPSendData(IN PIRP Irp, IN PIO_STACK_LOCATION IrpSp)
{
	...
	KdPrint(("[VMI_net] new_TCPSendData ~\n"));
...
process_request(ULONG ctl_code, char *buf, OUT ULONG *out_len, ULONG buf_size)
{
    ...
    *out_len = 0;
    ...
    if (buf == NULL)
    {
        KdPrint(("~![VMI_net] process_request() buf == NULL!\n"));
        return STATUS_UNSUCCESSFUL;
    }
...
tdi_set_event_handler(PIRP irp, PIO_STACK_LOCATION irps, struct _completion *completion)
{
    ...
    if (r_set_event->EventType < 0 || r_set_event->EventType > MAX_EVENT) {
        KdPrint(("[VMI_net] tdi_set_event_handler: unknown EventType %d!\n", r_set_event->EventType));
        return result;
    }
    ...
    if (r_set_event->EventHandler != NULL)
    {
        ...
        {
            KdPrint(("[VMI_net] tdi_set_event_handler[%s]: fltdevobj: 0x%x; FileObj 0x%x; EventType: %d\n",
                "(+)",
                irps->DeviceObject,
                irps->FileObject,
                r_set_event->EventType));

            KdPrint(("[VMI_net]  -tdi_set_event_handler: old_handler 0x%x; old_context 0x%x\n",
    ...
    if (r_set_event->EventType == TDI_EVENT_CONNECT)
    {
        ...
        if (ae == NULL)
        {
            ae = find_addr_entry(irps->FileObject, &irql);
            if (ae == NULL) {
                KdPrint(("~![VMI_net] tdi_set_event_handler: find_addr_entry(0x%x)!\n", irps->FileObject));
                return result;
            }
        }
        else
        ...
        if ( ((TDI_ADDRESS_IP *)(local_addr->Address))->in_addr != 0x100007f )
        {
            ...
            if (r == NULL) {
                KdPrint(("~!![VMI_net] tdi_set_event_handler: malloc_np\n"));
                goto done;
            }
