...
tdi_create(PIRP irp, PIO_STACK_LOCATION irps, PDEVICE_OBJECT old_devobj, int ipproto, OUT struct _completion *completion)
{
	...
	KdPrint(("[VMI_net] tdi_create: pid: 0x%x\n", pid));
    ...
    {
        KdPrint(("[VMI_net]\n[VMI_net] tdi_create: [addrobj]       fltdevobj: 0x%x; FileObj 0x%x\n",
            irps->DeviceObject,
            irps->FileObject));
        ...
        if (status != STATUS_SUCCESS) {
            KdPrint(("~![VMI_net] tdi_create: ot_add_fileobj: 0x%x\n", status));
            return FILTER_DENY;
        }
        ...
        if (completion->context == NULL) {
            KdPrint(("~![VMI_net] tdi_create: TdiBuildInternalDeviceControlIrp\n"));
            return FILTER_DENY;
        }
		//Set complete function,we can access the function to get IP address and port.This function is set to IRP complete function later.
        ...
        memcmp(ea->EaName, TdiConnectionContext, TDI_CONNECTION_CONTEXT_LENGTH) == 0)
    {
        ...
        KdPrint(("[VMI_net]\n[VMI_net] tdi_create: [connobj]       fltdevobj: 0x%x; connobj 0x%x; conn_ctx 0x%x\n",
            irps->DeviceObject,
            irps->FileObject,
            conn_ctx));
...
tdi_create_addrobj_complete(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context)
{
    ...
    KdPrint(("[VMI_net] tdi_create_addrobj_complete:fltdevobj: 0x%x; FileObj 0x%x\n",
		DeviceObject,
        irps->FileObject));

	if (status) {
		KdPrint(("~![VMI_net] tdi_create_addrobj_complete: status 0x%x\n", status));
		goto done;
	}
	...
	if (ctx == NULL) {
		KdPrint(("~!![VMI_net] tdi_create_addrobj_complete: malloc_np!\n"));
	...
	if (mdl == NULL) {
		KdPrint(("~![VMI_net] tdi_create_addrobj_complete: IoAllocateMdl!\n"));
	...
	if (old_devobj == NULL) {
		KdPrint(("[VMI_net] tdi_create_addrobj_complete: get_original_devobj!\n"));
	...
	if (status != STATUS_SUCCESS)
    {
        ...
        {
            KdPrint(("~![VMI_net] tdi_create_addrobj_complete: IoCallDriver: 0x%x\n", status));
...
tdi_query_addr_complete(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context)
{
	...
	if (ae == NULL) {
		KdPrint(("~![VMI_net] tdi_query_addr_complete: ot_find_fileobj(0x%x)\n",
			ctx->addrobj));
	...
	if (addr->AddressLength > sizeof(ae->local_addr)) {
		KdPrint(("~![VMI_net] tdi_query_addr_complete: address too long! (%u)\n",
			addr->AddressLength));
    ...
    KdPrint(("[VMI_net] tdi_query_addr_complete: address: %x:%u, proto: %d\n", 
...
tdi_associate_address(PIRP irp, PIO_STACK_LOCATION irps, struct _completion *completion)
{
    ...
    {
		KdPrint(("~![VMI_net] tdi_associate_address: ObReferenceObjectByHandle!\n"));
		goto done;
	}

    KdPrint(("[VMI_net] tdi_associate_address:      fltdevobj: 0x%x; connobj 0x%x ---> addrobj = 0x%x\n",
        irps->DeviceObject,
		irps->FileObject,
        addrobj));
	...
	if (ce == NULL) {
		KdPrint(("~![VMI_net] tdi_associate_address: find_conn_entry(0x%x)\n", irps->FileObject));
		goto done;
	}
...
{
	KdPrint(("[VMI_net] tdi_disassociate_address: connobj 0x%x\n", irps->FileObject));
