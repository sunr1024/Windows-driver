...
tdi_connect(PIRP irp, PIO_STACK_LOCATION irps, struct _completion *completion)
{
	...
	if (ipproto == IPPROTO_TCP)
    {
        ...
        if (ce == NULL) {
            KdPrint(("~![VMI_net] tdi_connect: find_conn_entry(0x%x)!\n", irps->FileObject));
            return FILTER_ALLOW;
        }
    ...
    else if (ipproto == IPPROTO_UDP)  // For UDP: connobj and addrobj are the same
    {
        ...
        KdPrint(("~![VMI_net] tdi_connect: unsupport proto: %d!\n", ipproto));
	...
	if (local_addr->AddressLength != remote_addr->AddressLength) {
		KdPrint(("~![VMI_net] tdi_connect: different addr lengths! (%u != %u)\n",
    ...
    KdPrint(("[VMI_net] tdi_connect: (pid:%u/%u) FileObj 0x%x: %x:%u -> %x:%u (ipproto = %d)\n",
    ...
    if (r == NULL) {
        KdPrint(("~!![VMI_net] tdi_connect: malloc_np\n"));
        goto done;
    }
...
tdi_connect_complete(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context)
{
	...
	if (Irp->IoStatus.Status == STATUS_SUCCESS)
    {
        ...
        if (param != NULL)
        {
            ...
            KdPrint(("[VMI_net] tdi_connect_complete: connobj 0x%x\n", irps->FileObject));
        ...
        {
            KdPrint(("~!![VMI_net] tdi_connect_complete: malloc_np!\n"));
            if (r)
                free(r);
        }
	}
    ...
    {
        KdPrint(("~![VMI_net] tdi_connect_complete: status 0x%x\n", Irp->IoStatus.Status));
...
update_conn_info(PDEVICE_OBJECT devobj, PFILE_OBJECT connobj, flt_request *pRequest)
{
	...
	if (query_irp == NULL) {
		KdPrint(("~![VMI_net] update_conn_info: TdiBuildInternalDeviceControlIrp!\n"));
		goto done;
	}

	param = (struct _connobj_request_localaddr *)malloc_np(sizeof(*param) + TDI_ADDRESS_INFO_MAX);
	if (param == NULL) {
		KdPrint(("~![VMI_net] update_conn_info: malloc_np!\n"));
		goto done;
	}
	...
	if (mdl == NULL) {
		KdPrint(("~![VMI_net] update_conn_info: IoAllocateMdl!\n"));
		goto done;
	}
...
update_conn_info_complete(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context)
{
	...
	if (status != STATUS_SUCCESS)
		KdPrint(("~![VMI_net] update_conn_info_complete: set_tcp_conn_local: 0x%x!\n", status));
    else
        if (param->pRequest)
        {
...
tdi_disconnect(PIRP irp, PIO_STACK_LOCATION irps, struct _completion *completion)
{
	...
	KdPrint(("[VMI_net] tdi_disconnect: connobj 0x%x (flags: 0x%x)\n",
		irps->FileObject,
        param->RequestFlags));
...
set_tcp_conn_localaddr(PFILE_OBJECT connobj, TA_ADDRESS *local)
{
    ...
    if (ce == NULL) {
        KdPrint(("~![VMI_net] set_tcp_conn_localaddr: find_conn_entry(0x%x)!\n", connobj));
    ...
    KdPrint(("[VMI_net] set_tcp_conn_localaddr: got CONNECT LOCAL %x:%u\n",
