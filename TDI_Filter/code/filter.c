...
quick_filter(flt_request *request, OUT struct _flt_rule *rule)
{
    ...
    {
		KdPrint(("~![VMI_net] quick_filter: not ip addr!\n"));
        return result;
    } 
	...
	for (r = g_rules.chain->head; r != NULL; r = r->next)
		...
		{
			result = r->result;
			KdPrint(("[VMI_net] quick_filter: found rule with result: %d\n", result));
...
add_flt_rule(int chain, const struct _flt_rule *rule)
{
	...
	if (new_rule == NULL) {
		KdPrint(("[VMI_net] add_flt_rule: malloc_np\n"));
...
set_chain_pname(int chain, char *pname)
{
	...
	KdPrint(("[VMI_net] set_chain_pname: setting name %s for chain %d\n", pname, chain));
...
set_pid_pname(ULONG pid, char *pname)
{
	...
	KdPrint(("[VMI_net] set_pid_pname: setting pname %s for pid %u\n", pname, pid));
	...
	for (i = 0; i < MAX_CHAINS_COUNT; i++)
		if (g_rules.chain[i].pname != NULL &&
			...
			KdPrint(("[VMI_net] set_pid_pname: found chain %d\n", i));
			chain = i;
...
{
    //KdPrint(("[VMI_net] InsertRequestList request: 0x%x\n", r));
