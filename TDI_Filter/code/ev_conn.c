    ...
    OUT PIRP *AcceptIrp) // done!
{
	...
	KdPrint(("[VMI_net] tdi_event_connect: addrobj 0x%x\n", ae->addrobj));
	...
	KdPrint(("[VMI_net] tdi_event_connect(pid:%u): %x:%u -> %x:%u\n",
		ae->pid,
    ...
    if (r == NULL) {
        KdPrint(("~!![VMI_net] tdi_event_connect: malloc_np\n"));
    ...
    {
        KdPrint(("~![VMI_net] tdi_event_connect: status from original handler: 0x%x\n", status));
		goto done;
    }
	...
	KdPrint(("[VMI_net] tdi_event_connect: connobj 0x%x\n", irps->FileObject));
	...
	if (local_addr->AddressLength != remote_addr->AddressLength) {
		KdPrint(("[VMI_net] tdi_event_connect: different addr lengths! (%u != %u)\n",
	...
	if (remote_addr->AddressLength > sizeof(ae->remote_addr)) {
		KdPrint(("[VMI_net] tdi_event_connect: address too long! (%u)\n",
			remote_addr->AddressLength));
    ...
    {
        KdPrint(("[VMI_net] tdi_event_connect: deny on reason 0x%x\n", status));
...
tdi_evconn_accept_complete(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context)
{
	...
	KdPrint(("[VMI_net] tdi_evconn_accept_complete: status 0x%x\n", Irp->IoStatus.Status));
    ...
    if (Irp->IoStatus.Status == STATUS_SUCCESS)
    {
        ...
        if (ucn_param != NULL)
        {
            ...
            KdPrint(("[VMI_net] tdi_evconn_accept_complete: connobj 0x%x\n", ctx->connobj));
            ...
            KdPrint(("~!![VMI_net] tdi_evconn_accept_complete: malloc_np!\n"));
    ...
    IN ULONG DisconnectFlags)
{
	...
	KdPrint(("[VMI_net] tdi_event_disconnect: (flags: 0x%x)\n",
        DisconnectFlags));
