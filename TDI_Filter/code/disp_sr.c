...
tdi_send(PIRP irp, PIO_STACK_LOCATION irps, struct _completion *completion)
{
	...
	struct _addr_entry *ae = { 0 };
	KIRQL irql;
    int ipproto;

	KdPrint(("[VMI_net] tdi_send: FileObj: 0x%x; SendLength: %u; SendFlags: 0x%x\n",
		irps->FileObject,
        param->SendLength,
        param->SendFlags));
...
tdi_receive(PIRP irp, PIO_STACK_LOCATION irps, struct _completion *completion)
{
	...
	KdPrint(("[VMI_net] tdi_receive: connobj: 0x%x; ReceiveLength: %u; ReceiveFlags: 0x%x\n",
...
tdi_receive_complete(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context)
{
	...
	KdPrint(("[VMI_net] tdi_receive_complete: connobj: 0x%x; status: 0x%x; received: %u\n",
		irps->FileObject,
        Irp->IoStatus.Status,
