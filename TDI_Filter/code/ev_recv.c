    ...
    OUT PIRP *IoRequestPacket)
{
	...
	KdPrint(("[VMI_net] tdi_event_receive: addrobj 0x%x; %u/%u; flags: 0x%x\n",
	...
	KdPrint(("[VMI_net] tdi_event_receive: status 0x%x; BytesTaken: %u; Irp: 0x%x\n",
...
tdi_client_irp_complete(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PVOID Context)
{
	...
	KdPrint(("[VMI_net] tdi_client_irp_complete: status: 0x%x; len: %u\n",
    ...
    if (b_call)
    {
        ...
        KdPrint(("[VMI_net] tdi_client_irp_complete: original handler: 0x%x; status: 0x%x\n",
            ctx->old_cr, status));
    }
    ...
    IN PVOID TsduDescriptor)
{
    ...
    KdPrint(("[VMI_net] tdi_event_chained_receive: connobj 0x%x; %4u; flags: 0x%x; status 0x%x\n",
        ae->connobj,
        ReceiveLength,
        ReceiveFlags,
        status));
